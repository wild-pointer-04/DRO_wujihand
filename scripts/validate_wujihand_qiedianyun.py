...
def get_partial_point_cloud(pc, ratio=0.5):
    """
    参考论文附录 C 的方法裁剪点云 [cite: 399-403]
    pc: [N, 3] 形状的张量或数组
    """
    # 1. 随机采样一个方向向量
    direction = torch.randn(3).to(pc.device)
    direction = direction / torch.norm(direction)
    
    # 2. 计算点云在方向上的投影
    # pc: [N, 3], direction: [3] -> projections: [N]
    projections = torch.matmul(pc, direction)
    
    # 3. 按照比例保留投影值较大的点 (模拟可见部分)
    num_points = pc.shape[0]
    num_keep = int(num_points * (1 - ratio))
    _, indices = torch.topk(projections, k=num_keep)
    
    partial_pc = pc[indices]
    
    # 4. 为了保持网络输入尺寸一致 (N=512)，通常需要重新采样
    # 这里可以使用随机重复采样或插值
    resample_indices = torch.randint(0, num_keep, (num_points,))
    return partial_pc[resample_indices]
...
def main():
    ...
    for i, data in enumerate(dataloader):
        ...
        for data_idx in tqdm(range(batch_size), desc=f"{robot_name}/{object_name}", leave=False):
            ...
            # === 执行裁剪逻辑 ===
            # 模拟真实场景：裁剪掉 50% 的点云 [cite: 293]
            object_pc = get_partial_point_cloud(full_object_pc.squeeze(0)).unsqueeze(0)
