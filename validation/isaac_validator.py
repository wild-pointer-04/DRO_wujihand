...
class IsaacValidator:
    ...
    #     is_filter=False,
    #     use_gui=False,
    #     robot_friction=3,#源码是3.0
    #     object_friction=3,#源码是3.0
    #     steps_per_sec=100,
    #     grasp_step=150,
    #     debug_interval=0.01
    # ):
    def __init__(
        ...
        # # ... (在 self.sim_params 设置部分)
        # self.sim_params.dt = 1 / steps_per_sec
        # # 🟢 [修改4] 增加子步数：处理复杂的多指接触，建议 5-8 之间
        ...
        # self.sim_params.physx.use_gpu = True
        # self.sim_params.physx.solver_type = 1
        # # 🟢 [修改5] 增加位置迭代次数：让接触解算更准确
        # self.sim_params.physx.num_position_iterations = 8
        # self.sim_params.physx.num_velocity_iterations = 1
        
        # # 🟢 [修改6] 减小接触偏移：从 0.01 改为 0.002 (2mm)
        # # 这会让手指真的“贴紧”物体表面才产生力，抓得更实。
    ...
    def create_envs(self):
        for env_idx in range(self.batch_size):
            ...
            # 1. 降低力度：从 1000 降到 400 (如果还炸就降到 100)
            robot_properties["stiffness"].fill(400.0) 
            
            # 2. 降低阻尼：配合刚度调整
            robot_properties["damping"].fill(37.0)

            # 3. 【关键】增加骨架惯性：这能极大抑制高频抖动！
            robot_properties["armature"].fill(0.001) 

            # 3. 限制最大力矩 (Effort Limit) [非常重要]
            # 即使刚度很高，如果限制了最大力矩，手指推物体的力也不会无限大
            # WujiHand 这种小手，通常给 1.0 - 2.0 就够了，太大绝对会炸
            #robot_properties["effort"].fill(1.5)
    ...
    def run_sim(self):
        ...
        num_fly = fly_mask.sum().item()
        num_drop = drop_mask.sum().item()

        # 🟢 修改：去掉 GPU 判断，强制刷新缓冲区
        # 使用 \r 尝试回到行首，或者直接打印
        msg = f"   [Debug] OK:{num_success} | Drop:{num_drop} | Fly:{num_fly}"
