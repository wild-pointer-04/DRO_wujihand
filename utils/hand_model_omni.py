...
class HandModel:
    ...
    def get_transformed_links_pc(self, q=None, links_pc=None):
        ...
        # 修改开始
        for link_index, (link_name, link_pc) in enumerate(links_pc.items()):
            # 1) 先从 frame_status 里拿对应的 transform
            status = self.frame_status.get(link_name, None)
            if status is None:
                # 说明这个 link 在 FK 里没有 frame，比如 hand_base_link 或 *_Virtual_Point
                # 直接跳过就好，不参与点云
                # print(f"[WARN] skip link with no FK frame: {link_name}")
                continue

            # 2) 保证 link_pc 是 tensor
            if not torch.is_tensor(link_pc):
                link_pc = torch.tensor(link_pc, dtype=torch.float32, device=q.device)
            ...
            # 3) 取变换矩阵
            se3 = status.get_matrix()[0].to(q.device)  # (4, 4)
        ...
        if len(all_pc_se3) == 0:
            # 极端情况：全被 skip 掉
            return torch.zeros(0, 4, device=q.device)
        #修改结束
